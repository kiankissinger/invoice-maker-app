...
export function renderDocumentHtml({ doc, client, profile, isPro }: RenderInput): string {
  ...
  const watermark = isPro
    ? ''
    : `<div class="watermark">Created with ${APP_NAME} — free plan</div>`;
...
export async function previewDocument(input: RenderInput): Promise<void> {
  await Print.printAsync({ html: renderDocumentHtml(input) });
}

export async function shareDocumentPdf(input: RenderInput): Promise<void> {
  const { uri } = await Print.printToFileAsync({ html: renderDocumentHtml(input) });
